...
TEST_CASE("JsonRpcServer concurrency", "[jsonrpc][concurrency]")
{
  ...
  // Slice-A review TA-2: register/unregister CONCURRENT WITH dispatch. The prior
  // section only fans out dispatch; this drives many writers mutating the handler +
  // options maps while many readers dispatch, to surface a raw map data race under
  // `setarch $(uname -m) -R` + -DIORA_ENABLE_TSAN=ON (removing the _mutex trips it).
  // Writers churn DISTINCT rotating names (never "increment") so reader outcomes stay
  // deterministic while the maps mutate underneath.
  SECTION("register/unregister concurrent with dispatch (TSan stress)")
  {
    ...
    for (int w = 0; w < writers; ++w)
    {
      writerThreads.emplace_back(
        [&, w]()
        {
          ...
          while (!stop.load())
          {
            const std::string name = "m" + std::to_string(w) + "_" + std::to_string((i++) & 7);
            server.registerMethod(name, [](const iora::parsers::Json &, RpcContext &)
                                  { return iora::parsers::Json::object(); });
            (void)server.hasMethod(name);
            (void)server.getMethodNames();
        ...
        [&]()
        {
          ...
          for (int i = 0; i < readsPerThread; ++i)
          {
            ...
            const std::string resp = server.handleRequest(
              R"({"jsonrpc":"2.0","method":"increment","params":{},"id":1})", ctx);
    ...
    // "increment" is never touched by the writers, so every reader dispatch succeeds
    // despite the concurrent map churn — a non-vacuous safe-publication assertion.
    REQUIRE(ok.load() == readers * readsPerThread);
    REQUIRE(counter.load() == readers * readsPerThread);
  }
}
...
// task-2.4: new coverage for the task-1.3 behavior change — RequestMetadata is
// held by value and RpcContext is copyable. A copy must carry authSubject and
// metadata, and mutating the copy must not affect the original (independent
// storage). These are new behaviors, so they need new tests rather than a ported
// one. Mutation-tested: the copy-ctor deletion would fail to compile, and shared
// metadata storage would trip the independence assertions.
TEST_CASE("RpcContext is copyable and copies are independent (task-1.3/D-3)", "[jsonrpc][rpccontext]")
{
  static_assert(std::is_copy_constructible<RpcContext>::value,
                "RpcContext must be copy-constructible (task-1.3)");
  static_assert(std::is_copy_assignable<RpcContext>::value,
                "RpcContext must be copy-assignable (task-1.3)");
  static_assert(std::is_move_constructible<RpcContext>::value,
                "RpcContext must be move-constructible (task-1.3)");
  static_assert(std::is_move_assignable<RpcContext>::value,
                "RpcContext must be move-assignable (task-1.3)");

  SECTION("Copy constructor carries authSubject and metadata")
  {
    RpcContext original(std::string{"alice"});
    original.metadata().clientId = "10.0.0.1";
    original.metadata().method = "foo";
    original.metadata().requestSize = 42;

    RpcContext copy = original;
    REQUIRE(copy.authSubject().has_value());
    REQUIRE(copy.authSubject().value() == "alice");
    REQUIRE(copy.metadata().clientId == "10.0.0.1");
    REQUIRE(copy.metadata().method == "foo");
    REQUIRE(copy.metadata().requestSize == 42);
  }

  SECTION("Mutating a copy's metadata does not affect the original")
  {
    RpcContext original(std::string{"bob"});
    original.metadata().clientId = "10.0.0.1";
    original.metadata().requestSize = 42;

    RpcContext copy = original;
    copy.metadata().clientId = "192.168.1.1";
    copy.metadata().requestSize = 99;
    copy.metadata().method = "changed";

    // The original is untouched — metadata is stored by value, not shared.
    REQUIRE(original.metadata().clientId == "10.0.0.1");
    REQUIRE(original.metadata().requestSize == 42);
    REQUIRE(original.metadata().method.empty());
    // And the copy holds the mutated values.
    REQUIRE(copy.metadata().clientId == "192.168.1.1");
    REQUIRE(copy.metadata().requestSize == 99);
  }

  SECTION("Copy assignment carries authSubject and metadata independently")
  {
    RpcContext original(std::string{"carol"});
    original.metadata().clientId = "172.16.0.1";
    original.metadata().requestSize = 7;

    RpcContext assigned;
    assigned = original;
    REQUIRE(assigned.authSubject().has_value());
    REQUIRE(assigned.authSubject().value() == "carol");
    REQUIRE(assigned.metadata().clientId == "172.16.0.1");
    REQUIRE(assigned.metadata().requestSize == 7);

    assigned.metadata().requestSize = 1000;
    REQUIRE(original.metadata().requestSize == 7);
  }

  SECTION("A default-constructed context has no authSubject")
  {
    RpcContext ctx;
    REQUIRE_FALSE(ctx.authSubject().has_value());
  }
}
