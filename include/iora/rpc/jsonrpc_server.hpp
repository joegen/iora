...
{
public:
  /// \brief Construct a context with an optional authenticated subject.
  explicit RpcContext(std::optional<std::string> subject = {})
      : _authSubject(std::move(subject))
  {
  }

  /// \brief Copyable and movable. RequestMetadata is held by value (four fields),
  /// so there is no reason to forbid copying; tracker C requires a copy.
  RpcContext(const RpcContext &) = default;
  RpcContext &operator=(const RpcContext &) = default;
  RpcContext(RpcContext &&) noexcept = default;
  RpcContext &operator=(RpcContext &&) noexcept = default;
...
/// \note MEMORY ORDERING (Slice-A review TA-1 / D-4 / task-1.5): these six counters
/// use the implicit default seq_cst. They are pure monotonic statistics and guard NO
/// companion non-atomic data, so memory_order_relaxed would suffice functionally; the
/// seq_cst->relaxed change is deliberately DECLINED and NOT applied here — this is a
/// VERBATIM move (task-1.5), and the relaxed micro-optimization (an ARM-perf-only
/// concern) is owned by tracker C, which relocates ServerStats behind a shared_ptr.
/// Do not re-raise it as a defect: seq_cst is correct, just stronger than required.
struct ServerStats
{
