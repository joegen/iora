...
// LD-2: this header no longer depends on the iora umbrella service header (which
// defines the service singleton and would recreate the include cycle the migration
// removes). Include exactly the foundation headers the client uses.
...
// folding in mergeHeaders_ — cpp17 LOW-5).
...
/// namespace-scope (namespace rpc), so a shared_ptr member to it carries
...
struct Config
{
  ...
  /// now a public library header; tracker 2026-07-26-1 phase-7 makes ~HttpClient
  /// virtual (its task F-3) alongside this promotion, since a
...
private:
  ...
  friend struct ::iora::rpc::JsonRpcClientTestAccess;
...
public:
  ...
  void quiesce()
  {
    ...
    // STEP 2 — CANCEL: cancelInFlight() every pooled client so a request parked in
    // receiveSync/acquireLease unwinds. This is what makes STEP 3 finite. Needs
    // stable connection identity (phase 4, task-4.1c). LOCKING: this holds the pool
    // _mutex AND (via cancelInFlight) HttpClient::_mutex simultaneously — the ONE
    // permitted _mutex -> HttpClient::_mutex nesting; safe because cancelInFlight()
    // is non-blocking/non-joining and HttpClient never takes _mutex (lock-contract
    // rule (vi)). It does NOT take _quiesceMutex (that is STEP 1/STEP 3).
    {
...
private:
  ...
  /// \brief Whether the caller expects a result body on a 2xx (task-5b.0 seam).
  /// Threaded BY VALUE (stack-local) through sendJsonCompressedOrIdentity_ ->
  /// sendJsonWithRetries_ -> sendJson_ so the ordered status dispatch in sendJson_
  /// can decide, per call, whether a 2xx-with-empty-body is a silent success. It is
  /// NOT a member/thread_local (B-ts M-2): sendJson_ runs concurrently over one
  /// shared Impl, so a mutable member/thread_local would race or leak across calls.
  ///   * NoResultExpected — notify() / all-notification batch: ANY 2xx is success
  ///     WITHOUT parsing (task-5b.1; Slice-B review W-1/W-2, 2026-09-09). Per
  ///     JSON-RPC §4.1 a notification gets no Response, so its outcome is the HTTP
  ///     status alone; a lax server that nonetheless sends a body (empty OR not) has
  ///     that body IGNORED — never parse-gated into a client failure (a garbage body
  ///     must not fail a delivered notification) and never silently rejected. The
  ///     earlier "a 2xx WITH a body still parses / discard after parse" contract is
  ///     superseded (it made a garbage body throw and an all-notification-batch error
  ///     body a silent discard — inconsistent). See DD-notify-ignores-2xx-body.
  ///   * ResultExpected — call()/batch: a success Response MUST carry a result body
  ///     (JSON-RPC §5); a 2xx-empty falls through to parse, which rejects the empty
  ///     body, and call()'s id-correlation (task-5b.3) also rejects it (M6).
  ...
  /// \brief Run \p fn under the shared failure-accounting tail (Slice-B review
  /// L10): a PoolExhaustedError charges poolExhaustions + failedRequests; any other
  /// throw charges failedRequests; both RETHROW unchanged. The pre-charge
  /// (totalRequests, and batchRequests/notificationRequests) stays in the caller
  /// BEFORE this runs, and the success increment stays INSIDE \p fn AFTER the parse
  /// (M-12 ordering) so an error reply is never counted as both successful and
  /// failed. Handles Json / void / vector<Json> return types via decltype(fn()).
  /// Replaces the three byte-identical two-catch tails formerly in callCore_,
  /// notifyCore_, and callBatchCore_.
  template <class Fn> auto withFailureAccounting_(Fn &&fn) -> decltype(fn())
  {
    try
    {
      return fn();
                                ...
                                const std::vector<std::pair<std::string, std::string>> &headers)
  {
    ...
    return withFailureAccounting_(
      [&]
      {
        auto lease = acquire_(endpoint);
        // task-5b.3: capture the request id BEFORE building the envelope so it can be
        // correlated against the response id BEFORE parseResponseOrThrow_ strips result.
        const std::uint64_t requestId = nextId_();
        iora::parsers::Json req = makeRequestEnvelope_(method, params, requestId);
        iora::parsers::Json resp = sendJsonCompressedOrIdentity_(
          lease.client(), endpoint, req, mergeHeaders_(headers),
          ResponseExpectation::ResultExpected);
        // task-5b.3: correlate the response id with the request id before the result is
        // extracted, so a stray/duplicate/mis-routed reply cannot be returned as this
        // call's result. Matches the batch path (parseBatchResponseOrThrow_).
        correlateIdOrThrow_(resp, requestId);
        // Count success only AFTER the response parses cleanly: parseResponseOrThrow_
        // throws RemoteError on a JSON-RPC error envelope, which withFailureAccounting_
                   ...
                   const std::vector<std::pair<std::string, std::string>> &headers)
  {
    ...
    withFailureAccounting_(
      [&]
      {
        auto lease = acquire_(endpoint);
        iora::parsers::Json req = makeNotificationEnvelope_(method, params);
        // task-5b.1: a notification expects no result, so route the send with
        // NoResultExpected — a 204 / 200-with-empty-body is a success WITHOUT parsing
        // the empty body (the old path Json::parse'd "" and threw on EVERY successful
        // notification, W-H4). The provably-not-sent retry is retained (it skips only
        // the response parse, not the not-sent retry).
        (void)sendJsonCompressedOrIdentity_(lease.client(), endpoint, req, mergeHeaders_(headers),
                                            ResponseExpectation::NoResultExpected);
        _stats.successfulRequests.fetch_add(1, std::memory_order_relaxed);
      });
  }
                 ...
                 const std::vector<std::pair<std::string, std::string>> &headers)
  {
    ...
    // Shared failure-accounting tail (withFailureAccounting_, Slice-B review L10).
    // Without it a batch acquire_ failure — PoolExhaustedError, or phase 6's new
    // post-relock ClientShutdownError / std::logic_error — was counted NOWHERE, even
    // though batchRequests and totalRequests had already been charged above. The
    // success increment lives in sendBatchOnLease_ (after the batch parses).
    return withFailureAccounting_(
      [&]
      {
        auto lease = acquire_(endpoint);
                    ...
                    const std::vector<std::pair<std::string, std::string>> &headers)
  {
    ...
    // (BatchItem's 2-arg ctor makes a notification). Per JSON-RPC 2.0 §6 the server
    // MUST return nothing at all for such a batch (never an empty Array), so the
    // response is a 2xx-empty. Track whether any item expects a response; if none
    // does, the send uses NoResultExpected (so the 2xx-empty is a success, not a
    // parse error) and we skip parseBatchResponseOrThrow_ entirely.
    bool anyResponseExpected = false;
    for (const auto &item : items)
    {
      if (item.id.has_value())
      {
        batchReq.push_back(makeRequestEnvelope_(item.method, item.params, item.id.value()));
        anyResponseExpected = true;
      }
      else
      {
    ...
    const ResponseExpectation batchExpectation =
      anyResponseExpected ? ResponseExpectation::ResultExpected : ResponseExpectation::NoResultExpected;
    ...
    // Slice-B review L2: an all-notification batch expects no Response (§6). The
    // NoResultExpected send returned success on the 2xx-empty without parsing, so
    // skip parseBatchResponseOrThrow_ (which would reject the null/empty body as
    // "must be an array") and return one null per item — the same value the mixed
    // path's notification branch produces. Still counts as one successful batch.
    if (!anyResponseExpected)
    {
      _stats.successfulRequests.fetch_add(1, std::memory_order_relaxed);
      return std::vector<iora::parsers::Json>(items.size());
    }
  ...
  /// \brief Extract code/message/data from a JSON-RPC error object and throw
  /// RemoteError. Shared by the strict parse (parseResponseOrThrow_) and the lenient
  /// non-2xx envelope surfacing (throwForNonSuccessResponse_).
  [[noreturn]] static void throwRemoteError_(const iora::parsers::Json &err)
  {
  ...
  /// \brief task-5b.2: validate that \p resp is a JSON-RPC 2.0 Response and return
  /// its result (or throw RemoteError on a conformant error object). The old code
  /// threw only on 'error', returned resp["result"] if present, else 'return resp;'
  /// — so a bare 123, an object with NEITHER key, or one carrying BOTH was silently
  /// accepted as the result. Now require an object with jsonrpc=="2.0" and EXACTLY
  /// ONE of result/error, throwing JsonRpcError otherwise. Also called per item by
  /// parseBatchResponseOrThrow_, so this tightens batch validation too.
  static iora::parsers::Json parseResponseOrThrow_(iora::parsers::Json resp)
  {
    if (!resp.is_object())
    {
      throw JsonRpcError("JsonRpcClient: response is not a JSON-RPC 2.0 object");
    }
    const bool hasResult = resp.contains("result");
    const bool hasError = resp.contains("error");
    if (hasResult == hasError)
    {
      throw JsonRpcError(
        "JsonRpcClient: response must contain exactly one of 'result' or 'error'");
    }
    if (!(resp.contains("jsonrpc") && resp["jsonrpc"].is_string() &&
          resp["jsonrpc"].get<std::string>() == "2.0"))
    {
      throw JsonRpcError(
        "JsonRpcClient: response missing or invalid jsonrpc version (must be \"2.0\")");
    }
    if (hasError)
    {
      throwRemoteError_(resp["error"]);
    }
    return resp["result"];
  }

  /// \brief task-5b.3: correlate a single-call response id with the request id.
  /// The batch path matches by id; the single-call path never did, so a mis-routed
  /// or duplicated reply could be returned as this call's result. Rules:
  ///   * a response with no id -> correlation failure (a conformant Response echoes
  ///     the request id, §5);
  ///   * id:null on an ERROR response is EXEMPT (§5 permits null when the server
  ///     could not determine the id) — but id:null on a NON-error response fails;
  ///   * request ids are always numeric (nextId_): a non-null id that is not a
  ///     number equal to the request id (a string id, a wrong number) fails.
  /// Runs BEFORE parseResponseOrThrow_ so a NON-NULL mismatched id on an error
  /// response surfaces as a correlation failure, not that response's RemoteError
  /// (B-web L-2).
  static void correlateIdOrThrow_(const iora::parsers::Json &resp, std::uint64_t requestId)
  {
    if (!resp.is_object() || !resp.contains("id"))
    {
      throw JsonRpcError("JsonRpcClient: response carries no id to correlate with request " +
    ...
    if (id.is_null())
    {
      ...
      {
        return; // id:null is valid on an error whose id could not be determined.
      }
      throw JsonRpcError("JsonRpcClient: response id is null on a non-error response (request " +
                         std::to_string(requestId) + ")");
    }
    // Type-aware compare: a numeric request id can only match a numeric response id.
    if (!id.is_number_integer() || id.get<std::uint64_t>() != requestId)
    {
      throw JsonRpcError("JsonRpcClient: response id does not match request " +
                         std::to_string(requestId));
    }
  }
  ...
  /// \brief True iff \p j is a conformant JSON-RPC 2.0 error object:
  /// {"jsonrpc":"2.0","error":{"code":<int>,"message":<string>,...},...}. Deliberately
  /// stricter than parseResponseOrThrow_'s exactly-one-of check — a non-2xx body that
  /// merely looks JSON-ish (a proxy's {"error":"gateway timeout"}) is NOT surfaced as
  /// a RemoteError (task-5b.4 leniency).
  static bool isConformantJsonRpcError_(const iora::parsers::Json &j)
  {
    if (!j.is_object())
    {
      return false;
    ...
    return err.contains("code") && err["code"].is_number_integer() && err.contains("message") &&
           err["message"].is_string();
  }

  /// \brief task-5b.4: on a non-2xx, surface the endpoint's JSON-RPC error envelope as
  /// a RemoteError — LENIENTLY. HttpClient::parseJsonOrThrow throws a bare
  /// std::runtime_error("HTTP failed with status: N") on any non-2xx BEFORE parsing,
  /// discarding the endpoint's -32600/-32001 envelope. Here, ONLY when the body is a
  /// CONFORMANT JSON-RPC error object is it surfaced as RemoteError(code,message,data);
  /// an absent, non-JSON, or non-conformant body (an intermediary proxy/gateway
  /// commonly returns 429/408/502/504 with its OWN non-conformant JSON) falls back to
  /// the HTTP-status-based error. A decodeResponseContentEncoding_ failure on the error
  /// body is CAUGHT and folded into the status fallback (B-web M-3), so a corrupt
  /// error-body coding never masks the real status. Since Reversal B, a 429/408
  /// surfaces here like any other non-2xx (it is simply not retried in the migration).
  /// ALWAYS throws.
  [[noreturn]] void
  throwForNonSuccessResponse_(iora::network::HttpClient::Response &response) const
  {
    // Decode any Content-Encoding on the error body; a decode failure must not mask
    // the status, so fall straight through to the status fallback on any throw.
    bool decoded = true;
    try
    {
    ...
    if (decoded && !response.body.empty())
    {
      const auto pr = iora::parsers::Json::parse(response.body, iora::parsers::ParseLimits{});
      if (pr.ok && isConformantJsonRpcError_(pr.value))
      {
        throwRemoteError_(pr.value["error"]);
      }
    }
    // absent / non-JSON / non-conformant / undecodable -> HTTP-status fallback. Keep
    // the exact message parseJsonOrThrow would have produced so existing catchers see
    // no change.
  ...
  sendJson_(iora::network::HttpClient &http, const std::string &url,
            ...
            ResponseExpectation expectation = ResponseExpectation::ResultExpected)
  {
    RequestCompressionCtl_ ctl; // allow=false -> identity
    return sendJson_(http, url, payload, headerMap, ctl, expectation);
  }
                                ...
                                ResponseExpectation expectation = ResponseExpectation::ResultExpected)
  {
    ...
    if (ctl.prepared)
    {
      ...
      // task-5b.6 (D-5b6, fix_5): honour a caller-supplied Content-Type (mergeHeaders_
      // canonicalised it to the exact "Content-Type" key), self-setting the default
      // ONLY when the caller supplied none — so a caller's application/json-rpc reaches
      // the wire as exactly one line. Content-Type is NOT in the caller reject-list.
      if (sendHeaders.find("Content-Type") == sendHeaders.end())
      {
    ...
    {
      // task-5b.6 (D-5b6, fix_5): identity path WITH a caller Content-Type. postJson
      // forces application/json (setLibraryOwnedHeader), discarding the caller's value,
      // so route via the string-body post() — which honours the map's Content-Type —
      // carrying the caller's value verbatim. mergeHeaders_ already canonicalised it to
      // one "Content-Type" key, so exactly one line reaches the wire.
      response = http.post(url, payload.dump(), headerMap, 0);
    }
    else
    {
      // Batch / compression-disabled / the forceIdentity 415 re-entry, NO caller
      // Content-Type: unchanged postJson path (forces application/json). The retained
      // uncompressed body for a re-entry is `payload`.
      response = http.postJson(url, payload, headerMap, 0);
    }

    // ── task-5b.0: ordered status dispatch over the raw Response ─────────────────
    // (1) NoResultExpected (notify / all-notification batch) + ANY 2xx -> success
    // WITHOUT parsing (task-5b.1; Slice-B review W-1/W-2, 2026-09-09, DD-notify-
    // ignores-2xx-body). Per JSON-RPC §4.1 a notification gets no Response, so its
    // outcome is the HTTP status ALONE: a 204/200-empty is the normal success signal
    // (Json::parse("") would otherwise throw on every successful notification, W-H4),
    // and a lax server that ALSO sends a body has it IGNORED here — a garbage body must
    // not parse-gate a delivered notification into a failure, and an all-notification
    // batch's stray body must not be silently discarded-after-parse (both inconsistent
    // in the prior empty-only form). Non-2xx still falls through (step 2 compressed-415
    // latch, step 3 lenient error surfacing / provably-not-sent retry). ResultExpected
    // (call/mixed-batch) does NOT skip — a success Response MUST carry a result (§5),
    // so a 2xx-empty is caught at step (3b) (M6).
    if (expectation == ResponseExpectation::NoResultExpected && response.success())
    {
      return iora::parsers::Json(nullptr);
    }

    // (2) DP-4 / task-2.2: read a 415 on the RAW response BEFORE decode/parse — but
    // ONLY when we compressed (an identity 415, e.g. media-type, is not our coding
    // being refused and must flow to the non-2xx surfacing below). A non-conformant
    // 415 carrying Content-Encoding could otherwise make decodeResponseContentEncoding_
    // throw (malformed inflate) first, so the status read precedes both. Ordered
    // BEFORE (3) so a compressed 415 latches-and-retries rather than surfacing as a
    // RemoteError.
    ...
    // (3) task-5b.4: any other non-2xx surfaces the endpoint's JSON-RPC error envelope
    // (LENIENT) rather than parseJsonOrThrow's bare "HTTP failed with status: N", which
    // discarded the -32600/-32001 envelope. Always throws.
    if (!response.success())
    {
      throwForNonSuccessResponse_(response);
    }

    // (3b) Slice-B review L4 (M6 diagnostic) + C-1: a ResultExpected 2xx whose body is
    // EMPTY or WHITESPACE-ONLY is a success Response carrying no result, which JSON-RPC
    // 2.0 §5 forbids for a call (RFC 9110 §15.3.5 / §6.4.1 let a 204/200 carry no body).
    // Throw a SEMANTIC JsonRpcError here rather than letting step (4)'s Json::parse()
    // surface an opaque "failed to parse" runtime_error (a whitespace-only body, e.g.
    // "   ", is not .empty() but is just as resultless — C-1). NoResultExpected already
    // returned at (1), so this fires only for call()/mixed-batch.
    if (iora::core::StringUtils::trim(response.body).empty())
    {
      throw JsonRpcError(
        "JsonRpcClient: server returned a success status with no result body for a call "
        "or batch item that requires a JSON-RPC result");
    }

    // (4) any remaining 2xx WITH a body -> decode + parse. Response::success() is
    // 200-299, so 201/202/206-with-body parse too, not only 200 (L9).
                                ...
                                ResponseExpectation expectation = ResponseExpectation::ResultExpected)
  {
    try
    {
      return sendJsonWithRetries_(http, url, payload, headers, /*forceIdentity=*/false, expectation);
    }
    catch (const ContentCodingRejectedError &e)
    {
      latchOff_(e.origin());
      return sendJsonWithRetries_(http, url, payload, headers, /*forceIdentity=*/true, expectation);
    }
  }
                             ...
                             const std::vector<BatchItem> &originalItems)
  {
    ...
    // Create map of id -> response for efficient lookup. The client only ever
    // sends numeric ids (nextId_), so a conformant batch response echoes numeric
    ...
    for (const auto &respItem : batchResp)
    {
      ...
      {
        if (!respItem["id"].is_number_integer())
        {
          throw JsonRpcError("Batch response carries a non-integer id; this client sends only "
                             "numeric ids and cannot correlate it");
        }
...
private:
  ...
  // here — this class sits directly in namespace rpc.
  ...
  //  (vi)  _mutex and HttpClient::_mutex have EXACTLY ONE permitted nesting:
  //        _mutex (OUTER) -> HttpClient::_mutex (INNER), taken ONLY in the
  //        blocking-quiesce destructor's STEP 2, where a cancelInFlight() sweep
  //        runs under _mutex (the `for (kv : _pools) forEachClient(... c.cancelInFlight())`
  //        loop) and cancelInFlight() takes HttpClient::_mutex. CORRECTED
  //        (Slice-B thread-safety review, 2026-09-09): the prior text asserted the
  //        two are "NEVER HELD SIMULTANEOUSLY", which is factually false — STEP 2
  //        does hold both. That claim was load-bearing for deadlock-freedom, so a
  //        maintainer trusting it who added a reverse HttpClient::_mutex -> _mutex
  //        path would create an ABBA deadlock. The nesting is SAFE because:
  //          (a) cancelInFlight() is non-blocking and NON-JOINING — it only sets
  //              _closing, notify_all()s, enqueues non-blocking Transport::close()
  //              on each cached session, and clears _connections. It NEVER joins an
  //              I/O or DNS thread (that is ~HttpClient -> cleanup()/stop(), which
  //              is exactly what clause (vi-bis) below keeps OUT of _mutex), so it
  //              cannot stall while holding HttpClient::_mutex under _mutex; and
  //          (b) HttpClient NEVER acquires the pool _mutex (it holds no reference to
  //              the pool), so no reverse edge exists and the single nesting cannot
  //              form a cycle. A request parked in receiveSync/acquireLease holds
  //              HttpClient::_mutex only transiently (the blocking waits run with it
  //              released), so cancelInFlight() does not block on it either.
  //  (vi-bis) COLLECT-THEN-DESTROY (task-4.1b), a SEPARATE absolute prohibition
  //        that survives (vi): ~HttpClient -> cleanup() takes HttpClient::_mutex AND
  //        JOINS the transport and DNS I/O threads (blocking), so it must NEVER run
  //        under _mutex. Every erase path MOVES its owning PooledConnection/
  //        EndpointPool reference into an evicted bin the caller destroys ONLY AFTER
  //        releasing _mutex. makeHttpClient_ (whose factory/configurer may call
  //        HttpClient::setTlsConfig, taking HttpClient::_mutex) likewise runs
  //        OUTSIDE _mutex since task-6.1/6.2. This is why the ONLY inner-lock site
  //        left under _mutex is the non-joining cancelInFlight() sweep of (vi).
  // ==========================================================================
  std::mutex _mutex;
...
{
public:
  /// \brief Construct a client over an externally-owned ThreadPool. LD-2: the
  /// former leading service-reference parameter (dead state — the client drives its
  /// own HttpClient instances and never touched the service) is removed.
