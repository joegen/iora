...
int main(int argc, char** argv)
{
  ...
  std::unordered_map<std::string, iora::core::Json> results;
  std::mutex resultsMutex;

  // Register EventQueue handler for processing summarization requests (fluent)
  svc.onEvent("summarize").handle([&](const iora::core::Json& input) {
  ...
  svc.on("/summarize").handleJson([&](const iora::core::Json& input) -> iora::core::Json {
    std::string text = input["text"];
    int maxTokens = input.value("max_tokens", 256);
  ...
  svc.on("/status").handleJson([&](const iora::core::Json& input) -> iora::core::Json {
    std::string requestId = input["requestId"];
