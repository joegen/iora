...
void locateSipsServer(DnsClient &client)
{
  ...
  // all paths (RFC 3263 §4.1) and defaults the A/AAAA fallback to TLS/5061 — so every target
  // returned is already a secure SIP target; no manual post-filter is needed.
  dns::ServiceResolutionResult result =
