...
| 1.13 | 2026-09-26 | **Engine allocate-then-connect split — `allocateSid()` + `connectWith()` (register-before-connect)** (tracker 2026-09-25-18). New additive engine/Transport primitives: `allocateSid()` mints a `SessionId` WITHOUT publishing anything engine-discoverable (no connecting-registry insert, no command enqueued) so the sid is not yet `sessionSendable` — a `send()` before `connectWith()` is rejected, never dropped; `connectWith(sid, host, port, tls, opts)` then inserts the connecting-registry entry and enqueues the connect command LAST. This lets a caller register state against the sid BEFORE the connect can fail: the register write is carried to the I/O thread by the command-queue mutex's release/acquire edge (the `_cmdMutex`/`_qmx` happens-before; the `eventFd` write is a wakeup only), so a connect failure is delivered to an already-registered sid (the enabler for iora_sip per-session connect-error scoping). On enqueue-fail/closed-queue/throw, `connectWith()` rolls the reservation back and returns err with **no** `onClose` — err XOR onClose for a registered sid; single-shot per sid (a second `connectWith()` for an in-use sid is rejected). The sid-agnostic `connect()` (both engines) and UDP `connectViaListener()` are unchanged (`connect()` is re-expressed through the same shared enqueue helper with the single-shot check disabled). Added on `ITransport`/`EngineBase` as NON-pure virtuals (unsupported default) so existing implementors are unaffected; `send()`/`sendAsync()` signatures unchanged. `connectViaListener()` is intentionally NOT split (connectionless / ICMP path). |
| 1.12 | 2026-09-25 | **UDP connect-then-send parity + structured `NotConnected` + race-free `observe()`** (tracker 2026-09-24-2, Slice A-ext). (1) A sid returned by the UDP engine's `connect()`/`connectViaListener()` is immediately sendable (the `sessionSendable`/`isSessionLive` check now admits a single unified connecting-sid registry for both entry points); datagrams sent during a named-host resolve window are buffered in the pending-connect entry and replayed in order at session insert, with an overflow drop-**oldest** that always keeps **>=1** (unconditional `pop_front`, no `closeOnBackpressure` — a pre-establishment datagram is recovered by SIP retransmission + peer server-transaction absorption). Every UDP pre-insert terminal (resolve/connect/AF/cap/close/shutdown) is routed through one `preInsertTerminal` helper that clears the registry *happens-before* its `onClose`. (2) New `TransportError::NotConnected` (appended after `Unknown`, preserving every prior ordinal); a synchronous `sendAsync` to an unknown/closed/cap-rejected sid reports it (was `Socket`), via a single-decision send path in both engines. (3) `EngineBase::isSessionLive()` (pure virtual) backs a **race-free `observe()`**: the observe-vs-close ownership handoff delivers each observer **exactly once** (never zero, never twice), with a **sentinel-0** "already closed; not retained, not invoked" return when the engine is not running or the async owned-terminal post fails, and the owned terminal delivered **async** (never inline under a caller lock — the SSE upgrade path). (4) Drain uniformity: the session drain now reports `ShuttingDown` (was `Unknown "shutdown"`) on both engines; the UDP shutdown drain invokes residual `RunOnIo`/connect-terminal closures behind a `_qClosed`/`_eventFd` backstop guard so a throwing callback cannot strand a queue waiter. |
...
|     |-- Connection:     addListener, connect (2 overloads), connectViaListener, allocateSid, connectWith, close
...
| `ConnectResult` | `Result<SessionId, TransportErrorInfo>` | `connect()`, `connectSync()`, `connectViaListener()`, `connectSyncCancellable()`, `connectWith()` |
...
**Race-free exactly-once handoff (`observe()` vs. a concurrent close).** `observe()` registers the observer, releases `observerMutex`, then asks the engine `isSessionLive(sid)` (an open session **or** a sid still connecting, read under the engine's session lock). The engine clears a session's liveness *happens-before* it dispatches that session's `onClose`, so:

- **live** -- the close handler has not yet reached the observer section; it will fire this observer later. `observe()` returns the real id.
- **not live** -- `observe()` re-takes `observerMutex` and tries to reclaim its own entry from **both** observer maps. If the entry is still present the close handler did not take it, so **this call owns the terminal** and delivers it **asynchronously** on the I/O thread (`runOnIoThread`, reason `Unknown` / `"session already closed"`) -- **never inline on the caller** (so `observe()` is safe to call while holding a caller lock the observer also takes, e.g. the SSE upgrade path under `SseServer::_mutex`). If the entry is absent, the close handler already took it and will fire it.

Either way the observer fires **exactly once** -- never zero, never twice.

**Return-value contract.** `observe()` returns the real `ObserverId` when the observer is retained by the close handler, or when this call owns the terminal and the async post **succeeded** (the callback fires async, its entry already removed, so `unobserve(id)` returns `false` = "may be in flight"). It returns the **sentinel `0`** when the session is already closed **and** the terminal cannot be posted -- the engine is not running (before `start()`, or after `stop()`) or the post fails (queue closed): the callback is **not retained and not invoked**. A caller that must react to an already-closed session (e.g. `sse_stream`) checks for `0` and cleans up itself.
...
> **Send validation (CF-H1).** Both engines reject a `send` to a session that is neither present-and-open nor still connecting, via `sessionSendable(sid)`: `send()` returns `false` for an unknown or closed session instead of silently enqueuing a command the engine would later drop (which used to return `true`, masking a dead connection from SIP RFC 3263 failover). A synchronous `sendAsync` to such a sid reports the structured `TransportError::NotConnected` (was `Socket`). **TCP/TLS:** a sid returned by `connect()` is immediately sendable -- the bytes are buffered FIFO behind the connect until TCP-established (TLS: until handshake Open), a setup failure surfaces only via `onClose` (`Connect`/`TLSHandshake`/`Resolve`), and a stream write-queue overflow closes the session (`WriteBackpressure`). **UDP (connect-then-send parity, A-ext):** a sid returned by `connect()`/`connectViaListener()` is likewise immediately sendable -- datagrams sent during a named-host resolve window are **buffered in the pending-connect entry** and replayed in order once the session is registered; buffer overflow drops the **oldest** datagram but always keeps **>=1** (a datagram is fire-and-forget and a dropped pre-establishment copy is recovered by SIP retransmission + peer server-transaction absorption, so this is latency, not loss). A UDP setup failure surfaces via `onClose` (`Resolve`/`Connect`/`Config`/`ResourceLimit`). In both engines `true` means accepted, not delivered; a close queued before the send drops it (accepted TOCTOU). The mechanism detail lives in the [sync-lifecycle deep dive](transport_sync_lifecycle.md).
...
- **any -> `Disabled`:** `setReadMode` calls `engine->setReadEnabled(sid, false)`. On TCP the engine withholds `EPOLLIN` from the session's epoll interest (via `updateInterest`), so no further `recv()` syscall occurs for the session; during a TLS handshake `EPOLLIN` is forced on regardless, since handshake reads are protocol-level, not application data. On UDP many virtual sessions share one socket, so `setReadEnabled` is a no-op that returns `false` and arriving data is dropped in the dispatch handler.
  - **Peer-disconnect detection is preserved independently of the read-gate.** The TCP engine arms `EPOLLRDHUP` on every session (in `updateInterest` and the accept/connect masks), so a graceful peer FIN still fires `onClose(PeerClosed)` even though `EPOLLIN` is withheld. Without this a read-disabled session would never notice a client disconnect (a graceful FIN presents as `EPOLLRDHUP`/`EPOLLIN`-EOF, never `EPOLLHUP`). **Contract (DR-1):** a peer close while reads are disabled **terminates** the session and **discards** any unread buffered inbound bytes — they are not preserved for a later re-enable. The write-only SSE stream (`sse_stream.hpp` `upgradeToSse`) is the sole consumer of `Disabled`. (A TLS `close_notify` without a following TCP FIN is an exception: it arrives as `EPOLLIN` data, which is gated off, so it is not detected until the peer's stack RSTs or the connection is reaped.)
...
- **UDP connect-then-send is now supported (A-ext).** An async `connect()`/`connectViaListener()` followed immediately by `send()` is accepted: the sid is sendable via the connecting registry, and datagrams are buffered during any named-host resolve window and replayed in order once the session registers (buffer overflow drops oldest, keeps >=1). You no longer need to await `onConnect` (or use `connectSync`) purely to send. As on TCP, a setup failure then arrives via `onClose` (`Resolve`/`Connect`/`Config`/`ResourceLimit`), not as a `send()` failure; `send()` returning `true` still means accepted, not delivered.
