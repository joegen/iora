...
3. **Pipelining loop.** Locate `\r\n\r\n`; if absent, wait for more data. A header block larger than
   ...
   the same framing verdict as the strict parser** — now literally by calling the SAME shared helper,
   `iora::network::detail::decideRequestFraming` (also used by `HttpRequest::fromWireFormat`), so the
   framer and parser cannot drift on where a request ends (RFC 9112 §6.3 request smuggling); each layer
   only maps the shared verdict to its own disposition. Concretely the framer gathers every
   `Content-Length` field-line and the *last* `Transfer-Encoding` value (via the shared
   `detail::checkHeaderLineGrammar` + `detail::trimOws`), and emits **400 + close** (poisoning the
   connection) for any ambiguous verdict: an obs-fold / whitespace-before-colon header line, a conflicting
   (differing-value) duplicate `Content-Length`, `Content-Length` together with `Transfer-Encoding`, or a
   `Transfer-Encoding` whose final coding is not chunked. The shared verdict also validates a single
   `Content-Length` as `1*DIGIT` and converts it: an invalid **or out-of-range** value is **400 + close**
   (SRV-L1; no longer a bare close). The `MAX_BODY_SIZE` (10 MB) **413** cap is a framer-only disposition
   applied to the verdict's length (the strict parser does not bound its body by `Content-Length`).
   Chunked detection uses the token-aware `detail::isChunkedFinalCoding` (so `x-chunked` does not
   false-positive); a chunked request is then framed with `findChunkedRequestEnd`, which shares
   `detail::parseChunkSize` / `chunkDataStep` / `walkTrailerSection` with the strict decoder.
...
> detection (SRV-L4TE), and chunk-size subtraction bounds (SRV-H2) -- and, as of `2026-09-12-1`, that
> framer/parser agreement is now **structural**: both layers compute one `iora::network::detail`
> framing verdict (`decideRequestFraming` + shared `parseChunkSize`/`chunkDataStep`/`walkTrailerSection`/
> `checkHeaderLineGrammar`), so the framing-verdict / chunk-size / trailer-walker logic can no longer
> drift between `handleIncomingData`/`findChunkedRequestEnd` and `fromWireFormat`/`decodeChunkedRequestBody`
> (the out-of-range `Content-Length` verdict is now unified to 400 on both layers). Also carried from the v1.0 re-verify:
