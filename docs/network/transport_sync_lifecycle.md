...
| 1.4 | 2026-09-24 | **TCP connect-then-send + setup-close codes** (tracker 2026-09-24-1). The TCP engine's `sessionSendable` also admits a sid in its connecting registry, so a sid returned by `connect()` is immediately sendable (bytes buffered FIFO until established; TLS until Open); UDP unchanged. `connectSync` now also returns the engine's watchdog closes -- `Connect` + `ETIMEDOUT` (connect timeout, was `Timeout`) and `TLSHandshake` + `ETIMEDOUT` -- when they fire before the `connectSync` deadline. Lock edge `syncMutex -> {_sessionRwMutex; then _cmdMutex}` (sequential engine leaves) documented. Review round 1: `connectSyncCancellable` treats every connect-phase timeout (`isConnectPhaseTimeout`: `Timeout`, or `Connect`/`TLSHandshake` + `ETIMEDOUT`) as a slice timeout and keeps polling until its own deadline; the configuration table documents the `connectTimeout`/`handshakeTimeout` split and their sum; the TSan limitation is corrected. |
...
- On `op->done`: return `std::move(op->result)` -- `ok(sid)` from `onConnect`, or the engine's `onClose` error. For the TCP engine that includes its setup watchdogs when they fire before this deadline: `Connect` + `sysErrno` `ETIMEDOUT` (`"Connect timeout"`, `config.connectTimeout`) and `TLSHandshake` + `ETIMEDOUT` (`"TLS handshake timeout"`, `config.handshakeTimeout`, armed at TCP-established). A caller classifying connect timeouts must accept those as well as `Timeout`.
...
Both engines validate the target session synchronously in `send()` / `sendAsync()` via `sessionSendable(sid)` -- "present in `_sessions` **and** not `closed`", the same validity notion `doSend` uses, taken under the session read lock. The TCP engine additionally admits a sid in its **connecting registry** (`connect()` returned it and its setup is still pending), in the same shared-lock acquisition: its bytes are buffered FIFO behind the connect until TCP-established (TLS: until handshake Open), a setup failure surfaces only via `onClose`, and a stream write-queue overflow closes the session (`WriteBackpressure`). Completion means accepted, not delivered. If it fails, `sendAsync` fires the completion callback with `TransportError::Socket` ("session not connected") rather than enqueuing a Send command that `doSend` would silently drop while reporting false success (which would defeat SIP RFC 3263 failover).
...
- **Connect-then-send ordering.** On the TCP engine a send issued right after an async `connect()` passes `sessionSendable` and is delivered after establishment (a failed setup reports through `onClose`, and the send's bytes are dropped with it). On UDP a caller must still `await onConnect` before sending. For `connectSync`, parking until `onConnect` (4.3, including the UDP F-1 fix) guarantees the returned sid is registered in `_sessions`, so a subsequent `sendSync` passes `sessionSendable` on either engine. The check's residual close-racing-right-after window is the accepted narrow TOCTOU documented in the engine: it shrinks the false-OK window from "always" to a rare race and cannot be closed without holding `_sessionRwMutex` across enqueue+dispatch.
...
- **Do NOT send on a UDP session before its connect completes.** Await `onConnect` (or a successful `connectSync` return) before `sendSync`/`send`; a send to a not-yet-registered or closed UDP session fails `sessionSendable` (CF-H1) with `Socket` "session not connected". (A TCP sid returned by `connect()` is sendable immediately; a send to an unknown or closed TCP sid fails the same way.)
...
**Lock ordering.** The convention is `callbackMutex -> syncMutex -> observerMutex -> userDataMutex`, but in practice no path holds more than one Transport-level lock at a time (callbacks are copied under `callbackMutex` and released before `syncMutex` is taken, and vice-versa), so the order is a defensive convention, not a live constraint. **Transport -> engine edge:** `connectSync` holds `syncMutex` across `engine->connect()`, which takes the TCP engine's leaf `_sessionRwMutex` (connecting-registry insert), releases it, and then takes the leaf `_cmdMutex` (enqueue) -- sequential, never co-held. Engine locks are leaves and are never held while a Transport lock is acquired (engine callbacks run with no engine lock held), so `syncMutex -> {_sessionRwMutex; then _cmdMutex}` cannot close a cycle.
...
| `connectTimeout` | `30000` | ms | Engine-side connect-timeout timer for the TCP phase only (independent of the `connectSync` caller timeout); expiry closes `Connect` + `ETIMEDOUT`. |
| `handshakeTimeout` | `30000` | ms | Engine-side TLS handshake-timeout timer, armed at TCP-established; expiry closes `TLSHandshake` + `ETIMEDOUT`. The TLS setup budget is therefore `connectTimeout + handshakeTimeout`, so a `connectSync` caller timeout below that sum can return `Timeout` before either engine watchdog fires. |
...
| UDP `connectSync` parks until `onConnect` (short-circuit removed) | An early UDP return raced the enqueue-time `sessionSendable` check (CF-H1) on a subsequent send; parking guarantees the returned sid is registered and immediately usable (F-1). (The TCP engine now also admits connecting sids, so a TCP `connect()`-returned sid is usable without parking.) |
...
| CF-H1 `sessionSendable` synchronous send validation | Rejecting a send to an unknown/closed session (rather than enqueuing a Send that `doSend` silently drops as false success) preserves SIP RFC 3263 failover; the narrow close-racing TOCTOU is accepted (cannot close without holding the session lock across dispatch). The TCP engine admits connecting sids so connect-then-send needs no `onConnect` round-trip; a setup failure is reported by `onClose` with a deterministic setup code. |
...
| Every `doConnect` failure path fires `onClose` | Guarantees every `connect()`-returned sid receives a terminal event (exactly one), so `connectSync` never orphans a `pendingConnects` entry. The TCP engine's pre-insert terminals erase the connecting registry before invoking `onClose`, and a throwing user `onClose` is swallowed. |
...
| **CF-H1 send validation has a narrow TOCTOU.** `sessionSendable` checks presence-and-open (or, TCP, connecting) under the session read lock; a close racing right after the check -- a close queued before the send -- still enqueues a send that `doSend` drops. | Accepted: shrinks the false-OK window from "always" to a rare race; cannot be closed without holding `_sessionRwMutex` across enqueue + dispatch. |
...
| **TSan requires ASLR disabled.** ThreadSanitizer (and this host's GCC 12 libasan) need `personality(ADDR_NO_RANDOMIZE)`; run the sanitized binaries under `setarch -R`. | TSan is run that way for the TCP engine binaries (`build-tsan`, `iora_core` instrumented); it is not wired into CI. |
