...
|---------|------|---------|
| 1.2 | 2026-09-13 | Single-sourced the RFC 9112 §6.3 request-framing verdict + §7.1 chunk parsing into `iora::network::detail` (`decideRequestFraming`, `parseChunkSize`, `chunkDataStep`, `walkTrailerSection`, `trimOws`, `checkHeaderLineGrammar`, `parseFullUInt`, `isHexDigit`) — the SAME helpers `HttpServer`'s transport framer consumes, so the strict parser and the framer cannot drift on where a request ends. Behavior-preserving except one deliberate unification: a `1*DIGIT` `Content-Length` that overflows `uint64` is now **400 on the parser too** (previously accepted, since the parser never converted it) — matching the framer (tracker 2026-09-12-1). |
...
- **Shared request-framing helpers (`iora::network::detail`)** — `decideRequestFraming` (the reason-carrying RFC 9112 §6.3 body-framing verdict: dup-CL, CL+TE, non-final chunked, invalid/out-of-range `Content-Length`); `chunkDataStep` / `walkTrailerSection` (§7.1 chunk-data and trailer-section parsing as a tri-state `{Ok|NeedMore|Malformed}` that never throws); `parseChunkSize` (a `bool` chunk-size-token validity check — ext-strip, OWS, `1*HEXDIG`, base-16 convert with overflow rejection); `checkHeaderLineGrammar` (§5.1/§5.2); `trimOws` (§5.6.3 OWS); `parseFullUInt`; and `isHexDigit`. These are the SINGLE definition the strict parser (`fromWireFormat`/`decodeChunkedRequestBody`) and the transport framer (`HttpServer::handleIncomingData`/`findChunkedRequestEnd`) both compute their framing DECISION from; each layer maps the shared result to its own DISPOSITION (parser throws `HttpRequestError`; framer poisons the connection with 400+close, or waits on `NeedMore`). This is what makes the "framer and parser reach the same verdict" anti-smuggling property structural rather than hand-maintained.
...
sequenceDiagram
    ...
    Req->>Req: smuggling checks (conflicting/invalid/out-of-range Content-Length; TE+CL; TE not final-chunked -> 400)
...
| A single `1*DIGIT` `Content-Length` that overflows `uint64` (e.g. `≥ 2^64`) — unframeable per RFC 9110 §8.6 / RFC 9112 §6.3 | `fromWireFormat` via `detail::decideRequestFraming` | 400 |
...
**Note — active request-smuggling rejection (RFC 9112 §6.3 / §5.1).** The parser does **not** merely tolerate framing-ambiguous requests and lean on the server's own body loop — it now actively **rejects** the smuggling vectors before returning a request. During the header loop it accumulates the distinct `Content-Length` values (into a `std::set`) and records whether a `Transfer-Encoding` was seen, then after the loop it evaluates the shared `detail::decideRequestFraming` verdict (the SAME verdict the HTTP server framer computes) and throws `HttpRequestError(400)` for: conflicting duplicate `Content-Length` field-lines (two separate, differing values); a single invalid (non-`1*DIGIT`) `Content-Length` — including an upstream-combined `"5, 6"`, which is one field-line and therefore one map entry, rejected as a non-`1*DIGIT` value rather than as a duplicate; a single `1*DIGIT` `Content-Length` that overflows `uint64` (unframeable per RFC 9110 §8.6 — the verdict converts the value, so this is now rejected on the parser as well as the framer); `Transfer-Encoding` and `Content-Length` present together; a `Transfer-Encoding` whose final coding is not `chunked` (via `detail::isChunkedFinalCoding`, §5.4); and whitespace between a field-name and its colon. Identical duplicate `Content-Length` values collapse to one and are tolerated. This is in addition to — not a replacement for — the HTTP server framing the request body in its own header loop *before* calling `fromWireFormat`; the parsed map is still never the framing source, but the parser now closes the desync vectors at the source. (See the framing test in `iora_test_http_header_combining.cpp`.)

**Request-side de-chunking — `decodeChunkedRequestBody`.** When the verdict is `Chunked`, `fromWireFormat` decodes the body **strictly** (a server must not guess request boundaries) via `decodeChunkedRequestBody`, which drives the shared `detail::parseChunkSize` (chunk-size token) and `detail::chunkDataStep` / `detail::walkTrailerSection` (tri-state chunk-data and trailer parsing). Because the parser operates on a *complete* buffer, it maps BOTH the `NeedMore` and `Malformed` tri-states to `HttpRequestError(400)` — a complete message cannot be legitimately incomplete (the streaming server framer instead *waits* on `NeedMore`; §5.3 of the server guide). It strips chunk extensions, drops the trailer section (never merging trailers into the header map), rejects any octet after the terminating `0`-chunk (`unexpected data after terminating chunk`), and caps the **decoded** length at `MAX_CHUNKED_BODY_SIZE` (10 MB → 413). This shares its chunk primitives with `HttpServer::findChunkedRequestEnd`, so the two cannot drift on chunk framing.
...
| 7 | After the loop, via `detail::decideRequestFraming`: conflicting/invalid/out-of-range `Content-Length`, `Transfer-Encoding`+`Content-Length`, or `Transfer-Encoding` without a final `chunked` coding → 400. | `fromWireFormat` |
