...
- Typed getters `getBool()`, `getInt()`, `getDouble()`, `getString()`, `getArray()`, `getObject()` check the active alternative and throw `Json::type_error` (which derives from `std::runtime_error`) on a type mismatch -- NOT `std::bad_variant_access`. A caller may catch `Json::type_error` (or `std::runtime_error`/`std::exception`) to recover from a wrong-type access.
- `get<T>()` / `value<T>()` is a template. For floating-point `T` it accepts either a stored double or a stored int (converting), and throws `Json::type_error` if the value is neither. For integral `T` it forwards to `getInt()` (and thus throws `Json::type_error` on a non-int). `std::string`, `Array`, and `Object` are supported; anything else is a compile-time `static_assert`.
...
- **A wrong-type scalar getter throws `Json::type_error`** (IS-A `std::runtime_error`). Catch `Json::type_error` to recover, or guard with `isInt()`/`isString()`/etc. first. `get<T>()`, `at()`, and `size()` throw the same `Json::type_error` on a type mismatch; `at()` throws `Json::out_of_range` (IS-A `std::out_of_range`) on a missing key / out-of-range index.
