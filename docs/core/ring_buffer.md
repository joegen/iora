...
- **`nextPowerOfTwo(v)`.** The classic decrement / OR-shift-cascade / increment bit twiddle. `nextPowerOfTwo(0)` returns `1`, so a requested capacity of 0 silently becomes 1 (no assertion). The cascade's `v |= v >> 32` step is guarded behind `if constexpr (sizeof(std::size_t) > 4)` (commit `eec6356`), so it is only ever instantiated on a 64-bit `std::size_t` -- the prior 32-bit shift-width UB is handled, not merely noted (see section 12).
...
**Resolved audit item.** The producer-side load of the consumer-owned `_tail` was promoted `relaxed` -> `acquire` (commit `eec6356`, 2026-09-10); it now pairs with the consumer's `release` store of `_tail`, matching the canonical SPSC pattern on both endpoints (sections 4 and 12).
