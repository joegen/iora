...
- **A worker thread cannot be killed by a handler throw.** `EventQueue` runs `threadCount` worker threads (not one, and not a timer thread). Each handler call is wrapped in `try/catch(const std::exception &)` followed by a trailing `catch (...)` (added in commit `eec6356`); either case is counted only as a log line and swallowed -- the worker survives.
...
| Rely on `std::exception`-derived handler throws being caught and logged (the common case). | Rely on a non-`std::exception` handler throw crashing the process to surface a bug -- since commit `eec6356` it is caught by the trailing `catch (...)` and only logged, same as `std::exception` (see 12). |
